...
def format_rgb(color_name: str, fmt: str, original_val: str) -> str:
    ...
    if fmt == "0xhex":
        alpha = original_val[2:4] if original_val.startswith("0x") and len(original_val) == 10 else "ff"
        return f"0x{alpha}{r:02x}{g:02x}{b:02x}"
    if fmt == "rgb": return f"rgb({r}, {g}, {b})"
    if fmt == "rgba":
        alpha = "1.0"
        if m := re.search(r"rgba\([^,]+,[^,]+,[^,]+,\s*([0-9.]+)\)", original_val): alpha = m.group(1)
    ...
    if fmt in ("hsl", "hsla"):
        ...
        alpha = "1.0"
        if m := re.search(r"hsla\([^,]+,[^,]+,[^,]+,\s*([0-9.]+)\)", original_val): alpha = m.group(1)
...
class TextInputOverlay(ModalScreen[str | None]):
    ...
    def __init__(self, prompt: str, default: str) -> None:
        super().__init__()
        self.prompt_text, self.default_text = prompt, default
    ...
    @on(Input.Submitted)
    def handle_submit(self, event: Input.Submitted) -> None: event.stop(); self.dismiss(event.value)
    def action_cancel(self) -> None: self.dismiss(None)

class PickerScreen(ModalScreen[str | None]):
    BINDINGS = [Binding("up,k", "cursor_up", "Up"), Binding("down,j", "cursor_down", "Down"), Binding("escape", "cancel", "Cancel")]
    def __init__(self, title: str, options: list[str], hints: list[str]) -> None:
        super().__init__()
        self.picker_title, self.options, self.hints = title, options, hints
    ...
    def on_selected(self, event: OptionList.OptionSelected) -> None: self.dismiss(self.options[event.option_index])
...
class SearchScreen(ModalScreen[tuple[int, int] | None]):
    BINDINGS = [Binding("escape", "cancel", "Cancel"), Binding("down,j", "cursor_down", "Down"), Binding("up,k", "cursor_up", "Up")]
    ...
    @on(Input.Changed)
    def handle_input(self, event: Input.Changed) -> None: self._populate_list(event.value)

    def _populate_list(self, query: str) -> None:
        ol = self.query_one(OptionList); ol.clear_options(); self.results = []
        ...
        for tab_idx, tab_items in self.app.schema.items():
            ...
            for item_idx, item in enumerate(tab_items):
                ...
                if match:
                    ...
                    if item.hints: txt.append(f" - {item.hints[0]}", style=f"italic {self.app.theme_colors['muted']}")
                    ol.add_option(Option(txt)); self.results.append((tab_idx, item_idx))

    @on(OptionList.OptionSelected)
    def on_selected(self, event: OptionList.OptionSelected) -> None:
        if event.option_index is not None and event.option_index < len(self.results): self.dismiss(self.results[event.option_index])
...
class ConfigOptionList(OptionList):
    BINDINGS = [
        Binding("enter", "app.submit_current", "Action"), Binding("j,down", "cursor_down", "Down"),
        Binding("k,up", "cursor_up", "Up"), Binding("g", "scroll_top", "Top"), Binding("G", "scroll_bottom", "Bottom"),
        Binding("h,left,backspace", "app.adjust(-1)", "Adjust Down"), Binding("l,right", "app.adjust(1)", "Adjust Up"),
        Binding("r", "app.reset_item", "Reset"), Binding("R", "app.reset_all", "Reset Page"),
        Binding("ctrl+d,page_down", "page_down", "Page Down"), Binding("ctrl+u,page_up", "page_up", "Page Up"),
    ]
    ...
    def action_page_down(self) -> None:
        ...
        self.highlighted = min(self.option_count - 1, (self.highlighted or 0) + 10)
    def action_page_up(self) -> None:
        if self.option_count == 0: return
        self.highlighted = max(0, (self.highlighted or 0) - 10)
...
class ScrollIndicator(Label):
    ...
    def update_scroll(self, scroll_y: float, max_scroll_y: float, viewport_height: float, virtual_height: float) -> None:
        ...
        self.display = True; self._max_scroll_y = max_scroll_y; self._track_height = int(viewport_height) - 2
        if self._track_height < 1: self.update("▲\n▼"); return
    ...
    def on_mouse_down(self, event: events.MouseDown) -> None:
        ...
        else: self._dragging = True; self.capture_mouse(); self._jump_to_y(event.y, ol)

    def on_mouse_move(self, event: events.MouseMove) -> None:
        if self._dragging:
            try: ol = self.app.query_one(f"#list-{int(self.id.split('-')[1])}", ConfigOptionList)
            except Exception: return
            self._jump_to_y(event.y, ol)

    def on_mouse_up(self, event: events.MouseUp) -> None:
        if self._dragging: self._dragging = False; self.release_mouse()
...
class Shortcut(Label):
    def __init__(self, key_text: str, label: str, action_name: str | None = None) -> None:
        ...
        self.key_text, self.label_text, self.action_name = key_text, label, action_name

    def render(self) -> Text:
        txt = Text(); txt.append(f"[{self.key_text}] ", style=self.app.theme_colors["accent"]); txt.append(self.label_text, style=self.app.theme_colors["fg"])
        return txt
...
class FileLink(Label):
    path = ""
    def render(self) -> Text:
        txt = Text(); txt.append(" 󰈔 File: ", style=self.app.theme_colors["accent"]); txt.append(self.path, style=self.app.theme_colors["fg"] + " underline")
    ...
    def on_click(self, event: events.Click) -> None:
        ...
        try:
            expanded = Path(self.path).expanduser().resolve()
            expanded.parent.mkdir(parents=True, exist_ok=True)
            expanded.touch(exist_ok=True)
            
            if event.button == 1:
                subprocess.Popen(["xdg-open", str(expanded)], start_new_session=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            elif event.button == 3:
                editor_cmd = shlex.split(os.environ.get("VISUAL", os.environ.get("EDITOR", "nano")))
                with self.app.suspend(): subprocess.run([*editor_cmd, str(expanded)])
        except Exception:
            if hasattr(self.app, "notify_status"): self.app.notify_status("Error resolving path or launching editor.")
    ...
    def watch_status_msg(self, new_val: str) -> None:
        for bar in self.query("#status-bar"):
            for link in self.query("#file-link"):
                if new_val:
                    txt = Text(); txt.append(" Status: ", style=self.app.theme_colors["accent"]); txt.append(new_val, style=self.app.theme_colors["error"])
                    bar.update(txt); bar.display, link.display = True, False
                else:
                    bar.display, link.display = False, True
...
class DuskyTUI(App):
    ...
    #main-box { width: 100%; height: 100%; border: solid $primary 50%; border-title-color: $primary; border-title-style: bold; border-title-align: center; border-subtitle-color: $primary; border-subtitle-style: bold; border-subtitle-align: right; background: transparent; padding: 0 1; }
    ...
    .tab-arrow { width: 3; height: 1; content-align: center middle; background: $background; color: $primary; text-style: bold; display: none; }
    ...
    #modal-dialog, #picker-dialog, #search-dialog { width: 60; height: auto; background: transparent; border: round $primary; padding: 0; }
    #modal-content, #picker-content, #search-content { width: 100%; height: 100%; background: $background; padding: 1 2; }
    #search-list, #picker-list { height: 1fr; scrollbar-size: 0 0; background: transparent; border: none; }
    #search-list > .option-list--option, #picker-list > .option-list--option { padding: 0 1; background: transparent; transition: background 100ms linear; }
    #search-list > .option-list--option-hover, #picker-list > .option-list--option-hover { background: $primary 10%; }
    #search-list > .option-list--option-highlighted, #picker-list > .option-list--option-highlighted { background: $primary 20%; color: $text; text-style: bold; }
    ...
    BINDINGS = [
        Binding("q,ctrl+c", "quit", "Quit", priority=True), Binding("ctrl+f", "search", "Search", priority=True),
        Binding("tab", "next_tab", "Next Tab", priority=True), Binding("shift+tab", "prev_tab", "Prev Tab", priority=True),
        Binding("alt+1", "switch_tab(0)", "Tab 1", show=False), Binding("alt+2", "switch_tab(1)", "Tab 2", show=False),
        Binding("alt+3", "switch_tab(2)", "Tab 3", show=False), Binding("alt+4", "switch_tab(3)", "Tab 4", show=False),
        Binding("alt+5", "switch_tab(4)", "Tab 5", show=False), Binding("alt+6", "switch_tab(5)", "Tab 6", show=False),
    ...
    def __init__(self, engine: BaseEngine, schema: dict[int, list[ConfigItem]], tabs: list[str], title="Dusky Editor", theme_path: str | None = None, **kwargs):
        ...
        self.engine, self.schema, self.tabs, self.editor_title = engine, schema, tabs, title
        self.theme_path = Path(theme_path).expanduser().resolve() if theme_path else None
        
        # Instance-bound theme dictionary (Replacing global THEME)
        self.theme_colors = {"bg": "#111318", "fg": "#e1e2e9", "accent": "#a8c8ff", "error": "#ffb4ab", "warning": "#bdc7dc", "success": "#dbbce1", "muted": "#43474e"}
        if self.theme_path:
            loaded_theme = load_matugen_json(self.theme_path)
            if loaded_theme: self.theme_colors.update(loaded_theme)
        ...
        with Vertical(id="main-box"):
            with Horizontal(id="tab-bar"):
                ...
                with Horizontal(id="tabs-container"):
                    yield Tabs(*[Tab(name, id=f"tab-id-{i}") for i, name in enumerate(self.tabs)], id="tabs")
    ...
    async def on_mount(self) -> None:
        ...
        for tab_idx, items in self.schema.items():
            ...
            for idx, item in enumerate(items):
                ...
                if cache_key in state:
                    ...
                    if item.type_ == "bool": item.value = (raw_val == "true")
                    elif item.type_ in ("int", "float"):
                        try: item.value = float(raw_val) if item.type_ == "float" else int(float(raw_val))
                        except ValueError: pass
                    elif item.type_ in ("string", "picker", "cycle"):
                        item.value = raw_val[1:-1] if raw_val.startswith('"') and raw_val.endswith('"') else raw_val
                    else: item.value = raw_val
                ol.add_option(Option(self._build_option(item, is_highlighted=(idx == 0)), id=f"item_{tab_idx}_{idx}"))
        ...
        if self.theme_path: self.set_interval(0.5, self.watch_theme_file)
    ...
    def current_option_list(self) -> ConfigOptionList | None:
        try:
            if switcher_current := self.query_one(ContentSwitcher).current:
                return self.query_one(f"#list-{switcher_current.split('-')[1]}", ConfigOptionList)
    ...
    def check_tab_overflow(self) -> None:
        ...
        try:
            has_overflow = self._cached_tabs_container.max_scroll_x > 0
            if has_overflow:
                self._cached_tab_left.display = self._cached_tabs_container.scroll_x > 0.5
                self._cached_tab_right.display = self._cached_tabs_container.scroll_x < (self._cached_tabs_container.max_scroll_x - 0.5)
            else: self._cached_tab_left.display = self._cached_tab_right.display = False
        except Exception: pass
        ...
        try:
            ...
            if current_mtime > self.last_theme_mtime:
                ...
                if new_theme is not None:
                    ...
                    for tab_idx in range(len(self.tabs)):
                        try:
                            ...
                            for idx, item in enumerate(self.schema.get(tab_idx, [])):
                                is_hl = (idx == ol.last_highlighted_idx) and (self.current_option_list == ol)
                    ...
                    for link in self.query(FileLink): link.refresh()
        except OSError: pass

    def apply_theme_to_engine(self) -> None:
        self._theme_toggle = not getattr(self, "_theme_toggle", False)
        theme_name = "dusky_dyn_A" if self._theme_toggle else "dusky_dyn_B"
        custom_theme = Theme(
            name=theme_name, primary=self.theme_colors["accent"], secondary=self.theme_colors["muted"],
            background=self.theme_colors["bg"], surface=self.theme_colors["bg"], warning=self.theme_colors["warning"],
            error=self.theme_colors["error"], success=self.theme_colors["success"], foreground=self.theme_colors["fg"]
        )
        self.register_theme(custom_theme); self.theme = theme_name
    ...
    def _build_option(self, item: ConfigItem, is_highlighted: bool = False) -> Text:
        ...
        txt.append(f"{item.label:<35}", style=f"{self.theme_colors['fg']} bold" if is_highlighted else self.theme_colors["fg"])
        
        val_str = str(item.value)
        is_modified = val_str != str(item.default)
        
        if item.type_ == "action": txt.append("   ⚡ Execute Action", style=f"bold {self.theme_colors['warning']}")
        else:
            txt.append("●  ", style=self.theme_colors["error"] if is_modified else self.theme_colors["muted"])
            if item.type_ == "bool": txt.append(" ◉ ON  " if item.value else " ◯ OFF ", style=f"bold {self.theme_colors['bg']} on {self.theme_colors['success']}" if item.value else f"{self.theme_colors['muted']} on {self.theme_colors['bg']}")
            elif item.type_ == "string": txt.append(f"[✎] {val_str}" if val_str else "[✎] Unset", style=self.theme_colors["accent"] if val_str else f"italic {self.theme_colors['muted']}")
            elif item.type_ == "picker": txt.append(f"[+] {val_str}", style=self.theme_colors["accent"])
            elif item.type_ == "color":
                r, g, b = color_to_rgb(val_str)
                txt.append(" ⬤ ", style=f"#{r:02x}{g:02x}{b:02x}")
                txt.append(f"{get_color_name(r, g, b)}", style=self.theme_colors["accent"])
            else: txt.append(val_str, style=self.theme_colors["fg"])
            if is_modified and is_highlighted: txt.append("   ↩ Reset", style=f"italic {self.theme_colors['error']}")
        return txt

    @on(Tabs.TabActivated)
    def handle_tab_activated(self, event: Tabs.TabActivated) -> None:
        try:
            self.query_one(ContentSwitcher).current = f"tab-{event.tab.id.split('-')[-1]}"
            event.tab.scroll_visible(animate=True, top=False)
            if ol := self.current_option_list:
                ol.focus(); self._update_pagination(ol); self._update_scroll_indicators()
        except Exception: pass

    @on(OptionList.OptionHighlighted)
    def handle_option_highlight(self, event: OptionList.OptionHighlighted) -> None:
        ol = event.option_list
        if not isinstance(ol, ConfigOptionList): return
        try: tab_idx = int(ol.id.split("-")[1])
        except (AttributeError, IndexError, ValueError): return
        
        if (last_idx := ol.last_highlighted_idx) is not None and last_idx != event.option_index:
            try: ol.replace_option_prompt_at_index(last_idx, self._build_option(self.schema[tab_idx][last_idx], False))
            except (IndexError, KeyError): pass
                
        if event.option_index is not None:
            try:
                ol.replace_option_prompt_at_index(event.option_index, self._build_option(self.schema[tab_idx][event.option_index], True))
    ...
    def _update_pagination(self, ol: ConfigOptionList) -> None:
        ...
        self.query_one("#main-box").border_subtitle = f" {idx + 1}/{ol.option_count} " if ol.option_count else " 0/0 "

    def _update_scroll_indicators(self) -> None:
        try:
            if not (switcher_current := self.query_one(ContentSwitcher).current): return
            tab_idx = int(switcher_current.split("-")[1])
            ol = self.query_one(f"#list-{tab_idx}", ConfigOptionList)
    ...
    def _commit_change(self, item: ConfigItem, new_val: Any) -> bool:
        """The Master Bridge: Handles conversion and passes strict data to the engine contract."""
    ...
    def action_search(self) -> None:
        def check_reply(result: tuple[int, int] | None) -> None:
            if result:
                ...
                try:
                    ol = self.query_one(f"#list-{tab_idx}", ConfigOptionList); ol.focus()
                    ol.highlighted = item_idx; ol.scroll_to_highlight()
                    item = self.schema[tab_idx][item_idx]
                    if ol.last_highlighted_idx is not None and ol.last_highlighted_idx != item_idx:
                        ol.replace_option_prompt_at_index(ol.last_highlighted_idx, self._build_option(self.schema[tab_idx][ol.last_highlighted_idx], False))
    ...
    def action_adjust(self, direction: int) -> None:
        ...
        try:
            if not (switcher_current := self.query_one(ContentSwitcher).current): return
            tab_idx, item_idx = int(switcher_current.split("-")[1]), ol.highlighted
            ...
            match item.type_:
                ...
                case "int" | "float":
                    new_val = item.value + (direction * (item.step or 1))
                ...
                case "color":
                    ...
                    try: idx = CYCLE_COLORS.index(get_color_name(r, g, b))
                    except ValueError: idx = 0
                    next_name = CYCLE_COLORS[(idx + direction) % len(CYCLE_COLORS)]
                    new_val = format_rgb(next_name, parse_color_format(str(item.value)), str(item.value))
                case _: return
    ...
    def action_reset_all(self) -> None:
        try:
            ...
            if ol := self.current_option_list:
                for idx, item in enumerate(items): 
                    ol.replace_option_prompt_at_index(idx, self._build_option(item, idx == ol.highlighted))
            if success_count > 0: 
    ...
    def action_submit_current(self) -> None:
        ol = self.current_option_list
        if ol and ol.highlighted is not None:
            ol._last_click_x, ol._mouse_down_highlight = 0, None
    ...
    def handle_selection(self, event: OptionList.OptionSelected) -> None:
        ...
        if isinstance(ol, ConfigOptionList):
            if getattr(ol, "_mouse_down_highlight", None) == event.option_index: self._handle_item_action(ol, event.option_index)
            ol._mouse_down_highlight, ol._last_click_x = None, 0

    def _handle_item_action(self, ol: ConfigOptionList, index: int) -> None:
        try: tab_idx, item = int(ol.id.split("-")[1]), self.schema[int(ol.id.split("-")[1])][index]
        except (AttributeError, IndexError, ValueError, KeyError): return
            
        if str(item.value) != str(item.default) and item.type_ != "action":
            width = self._build_option(item, True).cell_len
            if (width - 10) <= getattr(ol, "_last_click_x", 0) <= width + 2:
            ...
            case "int" | "float" | "string" | "color":
                def check_reply(new_val: str | None) -> None:
                    if new_val is not None:
                        if item.type_ in ("int", "float"):
                            try:
                                parsed = float(new_val) if item.type_ == "float" else int(new_val)
                                if item.min_val is not None: parsed = max(type(parsed)(item.min_val), parsed)
                                if item.max_val is not None: parsed = min(type(parsed)(item.max_val), parsed)
                                new_val = parsed
                            except ValueError: self.notify_status("Type Error"); return
                        if self._commit_change(item, new_val): ol.replace_option_prompt_at_index(index, self._build_option(item, True))
                self.push_screen(TextInputOverlay(f"Enter new {item.label}:", str(item.value)), check_reply)
            case "action":
                if item.key == "demo_sudo": self.notify_status("Acquiring Sudo... Simulated daemon restart.")
                else: self.notify_status(f"Executed: {item.label}")
            case "picker":
                def check_picker(new_val: str | None) -> None:
                    if new_val is not None and self._commit_change(item, new_val): ol.replace_option_prompt_at_index(index, self._build_option(item, True))
                self.push_screen(PickerScreen(item.label, item.options, item.hints), check_picker)
