...
void main() {
    ...
    if (u_style == 5 || u_style == 6) { // radial or circle
        ...
        float norm_idx = mod((ang + PI * 0.5) / (2.0 * PI), 1.0);
        ...
        if (u_style == 5) { // radial
            ...
            float norm_r = dist / max(1.0, target_r);
...
def on_gl_realize(widget):
    ...
    GL.glVertexAttribPointer(0, 2, GL.GL_FLOAT, GL.GL_FALSE, 2 * ctypes.sizeof(GL.GLfloat), ctypes.c_void_p(0))
...
def on_gl_render(widget, context):
    ...
    style_map = {"bars": 0, "segments": 1, "dots": 2, "wave": 3, "line": 4, "monitor": 4, "radial": 5, "circle": 6}
    GL.glUniform1i(GL.glGetUniformLocation(gl_shader_program, "u_style"), style_map.get(style_str, 0))
    GL.glUniform1i(GL.glGetUniformLocation(gl_shader_program, "u_bars"), min(64, n))
    GL.glUniform1i(GL.glGetUniformLocation(gl_shader_program, "u_segments_count"), config.get("segments_count", 16))
    GL.glUniform1f(GL.glGetUniformLocation(gl_shader_program, "u_thickness"), config.get("thickness", 0.5))
    GL.glUniform1i(GL.glGetUniformLocation(gl_shader_program, "u_shape_rounded"), 1 if config.get("shape_rounded", True) else 0)
    
    pos_str = config.get("position", "top")
    pos_map = {"top": 0, "center": 1, "bottom": 2}
    GL.glUniform1i(GL.glGetUniformLocation(gl_shader_program, "u_position"), pos_map.get(pos_str, 0))
    
    fade_str = config.get("fade_direction", "fade_to_base")
    fade_map = {"fade_to_base": 0, "fade_to_tip": 1, "solid": 2}
    GL.glUniform1i(GL.glGetUniformLocation(gl_shader_program, "u_fade_direction"), fade_map.get(fade_str, 0))
    GL.glUniform1f(GL.glGetUniformLocation(gl_shader_program, "u_fade_amount"), config.get("fade_amount", 1.0))
    GL.glUniform1f(GL.glGetUniformLocation(gl_shader_program, "u_resolution_x"), float(widget.get_allocated_width()))
    GL.glUniform1f(GL.glGetUniformLocation(gl_shader_program, "u_resolution_y"), float(widget.get_allocated_height()))
...
def setup_window():
    ...
    if config.get("glass_blur", True):
        os.system('hyprctl eval "hl.layer_rule({ match = { namespace = \'dusky-visualizer\' }, blur = true, ignore_alpha = 0.5 })" >/dev/null 2>&1 &')
    else:
        os.system('hyprctl eval "hl.layer_rule({ match = { namespace = \'dusky-visualizer\' }, blur = false })" >/dev/null 2>&1 &')
...
def tick():
    ...
    is_active = any(v > 0.001 for v in cava_data) or any(v > 0.0001 for v in smoothed_data)
