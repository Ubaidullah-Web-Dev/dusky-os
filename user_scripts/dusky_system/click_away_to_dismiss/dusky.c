...
void init_wayland_grab(void *gtk_window_ptr, ClearedCallback cb) {
  ...
  struct wl_surface *wl_surface =
      gdk_wayland_surface_get_wl_surface(gdk_surface);
  ...
  if (!custom_queue) {
    ...
    // Assign the registry proxy to our custom queue to avoid GTK main loop
    // races
    ...
    if (!grab_manager) {
      fprintf(stderr, "[libduskygrab] Error: hyprland_focus_grab_manager_v1 "
